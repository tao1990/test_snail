<?php
    ...
    function snail_alipay_create_order($orderSn,$amount,$subject,$body=""){
        ...
        $bizcontent = json_encode([
            'body'=>$body,
            'subject'=>$subject,
            'out_trade_no'=>$orderSn,//此订单号为商户唯一订单号
...
//                        . "}";
    ...
    function snail_wxpay_create_order($orderSn,$amount){
        ...
        $wxpay->config = array(
            'appid' => WX_APPID, /*微信开放平台上的应用id*/
            'mch_id' => WX_MCHID, /*微信申请成功之后邮件中的商户id*/
            'api_key' => WX_API_KEY, /*在微信商户平台上自己设定的api密钥 32位*/
        );
        
        $wxpay->notify_url = NOTIFY_WX;
        $res = $wxpay->Wxpay($amount,$orderSn); //调用weixinpay方法
        if($res){
            //return array('payCode'=>$res['prepayid'],'sign'=>$res['sign']);
            return $res;
        }else{
            return false;
        }
