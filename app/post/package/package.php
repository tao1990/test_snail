 ...
 *     description="body" ,example = "{	'uid':'','type':'','company':'','company_info':'','company_business':'','company_city':'','contacts_man':'','contacts_mobile':''}"
 *   ),
 * @SWG\Response(
 *   response=200,
 *   description="ok response",
...
if($ac == 'create'){
  ...
  if(tokenVerify($token)){
    ...
    if( $arr['uid'] == 0 || !$arr['type'] || !$arr['company'] || !$arr['contacts_man'] || !$arr['contacts_mobile']){
        header('HTTP/1.1 400 ERROR');
...
function createPackage($arr){
  ...
  if($insert_id){
        $sql="INSERT INTO `snail_post_log` (post_id,post_type,uid,dateline) VALUES (".$insert_id.",'PACKAGE','".$arr['uid']."',$time)";
        $conn->query($sql);
