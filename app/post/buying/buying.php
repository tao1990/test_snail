...
/**
 * @SWG\Get(path="/app/post/buying/buying.php?ac=list", tags={"post"},
 *   summary="获取俄货代购列表(OK)",
 *   description="",
 *   @SWG\Parameter(name="type", type="string", required=true, in="query",example = "带货类型"),
...
/**
 * @SWG\Post(path="/app/post/buying/buying.php?ac=create", tags={"post"},
 *   summary="创建俄货代购(OK)",
 *   description="",
 *   @SWG\Parameter(name="body", type="string", required=true, in="formData",
 *     description="body" ,example = "{'token':'',	'uid':'','type':'','title':'',	'buying_city':'','content':'','logo':'','contacts_man':'','contacts_mobile':'','contacts_wechat':''}"
 *   ),
 * @SWG\Response(
 *   response=200,
 *   description="ok response",
...
if($ac == 'create'){
  ...
  if(tokenVerify($token)){
    ...
    $arr['buying_city'] = empty($bodyData['buying_city'])? '':$bodyData['buying_city'];
    ...
    if($arr['uid'] == 0 || !$arr['type'] || !$arr['title'] || !$arr['buying_city'] || !$arr['content'] || !$arr['contacts_man'] || !$arr['contacts_mobile']){
...
function createAd($arr){
  ...
  $time = time();
  $post_id = 0;
  $sql="INSERT INTO `snail_post_buying` (uid,type,title,buying_city,content,logo,contacts_man,contacts_mobile,contacts_wechat,status)
  VALUES (".$arr['uid'].",'".$arr['type']."','".$arr['title']."','".$arr['buying_city']."','".$arr['content']."','".$arr['logo']."','".$arr['contacts_man']."','".$arr['contacts_mobile']."','".$arr['contacts_wechat']."',0);";
 
  $conn->query($sql);
  $insert_id = $conn->insert_id;
  if($insert_id){
        $sql="INSERT INTO `snail_post_log` (insert_id,post_type,amount,uid,dateline) VALUES (".$insert_id.",'BUYING',".PRICE_200.",".$arr['uid'].",$time)";
        $conn->query($sql);
...
function getListByType($type,$page=1,$pageCount=10){
    ...
    $total = $conn->query("SELECT * from `snail_post_buying` WHERE `status` = 1 AND `start_date` < $time AND `end_date` > $time $sqlStr;")->num_rows;
    $sql="SELECT * from `snail_post_buying` WHERE `status` = 1 AND `start_date` < $time AND `end_date` > $time $sqlStr ORDER BY id DESC limit $offset,$pageCount;";
    $result=$conn->query($sql);
    while ($row = mysqli_fetch_assoc($result))
    {
      $row2['id']       = $row['id'];
      $row2['typeCode']     = "BUYING";  
      $row2['title'] = $row['title'];
      $row2['logo'] = $row['logo'];
      $row2['typeName'] = $row['type'];
      $row2['buyingCity'] = $row['buying_city'];
